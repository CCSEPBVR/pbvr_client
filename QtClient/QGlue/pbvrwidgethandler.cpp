...
PBVRWidgetHandler::PBVRWidgetHandler() :
    kvs::oculus::jaea::WidgetHandler(),
    m_lefty_mode(false)
{
}

PBVRWidgetHandler::PBVRWidgetHandler(int width, int height) :
    kvs::oculus::jaea::WidgetHandler(width, height),
    m_showBoundingBoxFlag(false),
    m_showControlSphereFlag(true),
    m_lefty_mode(false)
{
}
...
void PBVRWidgetHandler::initializeStatus () {
    ...
    this->m_showBoundingBoxFlag = false;
    this->m_showControlSphereFlag = false;
}
...
//    if (pressed) {
//        std::cout << "PBVRWidgetHandler::drawCheckBox(" << label << ") : checkbox is pressed." << std::endl;
//    }

//    if (*enable) {
//        std::cout << "PBVRWidgetHandler::drawCheckBox(" << label << ") : enabled." << std::endl;
//    }

    if (ImGui::IsItemClicked()) {
//        std::cout << "PBVRWidgetHandler::drawCheckBox(" << label << ") : clicked." << std::endl;
        *enable = !(*enable);
    }
}

void PBVRWidgetHandler::updateSliders(int min, int max, int curTime, int minTime, int maxTime) {
    //    slider_min = min * SliderControl::stepSize;
    //    slider_max = max * SliderControl::stepSize;
    //    slider_val[0] = curTime * SliderControl::stepSize;
    //    slider_val[1] = minTime * SliderControl::stepSize;
    //    slider_val[2] = maxTime * SliderControl::stepSize;
        slider_min = min;
        slider_max = max;
...
//    if (updated[0]) {
//        std::cout << "time updated " << prev_val[0] << " -> " << slider_val[0] << std::endl;
//    }
//    if (updated[1]) {
//        std::cout << " min updated " << prev_val[1] << " -> " << slider_val[1] << std::endl;
//    }
//    if (updated[2]) {
//        std::cout << " max updated " << prev_val[2] << " -> " << slider_val[2] << std::endl;
//    }

    if (prev_val[0] != slider_val[0] ||
        prev_val[1] != slider_val[1] ||
        prev_val[2] != slider_val[2]) {

//        std::cout << "PBVRWidgetHandler::drawSlider() : call synchronizer." << std::endl;

        SliderSynchronizer::instance().synchronize(this, slider_min/SliderControl::stepSize, slider_max/SliderControl::stepSize, slider_val[0]/SliderControl::stepSize, slider_val[1]/SliderControl::stepSize, slider_val[2]/SliderControl::stepSize);
    }
}
...
static kvs::oculus::jaea::pbvr::PBVRWidgetHandler *instance = nullptr;
kvs::oculus::jaea::pbvr::PBVRWidgetHandler *PBVRWidgetHandlerInstance() {
    if (instance == nullptr) {
        instance = new kvs::oculus::jaea::pbvr::PBVRWidgetHandler ();
    }
    return instance;
}
