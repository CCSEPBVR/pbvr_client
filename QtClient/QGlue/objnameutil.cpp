...
bool ObjectNameUtil::isToIgnoreBoundingBox(std::string name) {
    return ignoreBoundingBoxNames.count(name) != 0;
}
bool ObjectNameUtil::isToIgnoreTranslationFactor(std::string name) {
    return ignoreTranslationFactorNames.count(name) != 0;
}
bool ObjectNameUtil::isToIgnoreModelingMatrix(std::string name) {
    return ignoreModelingMatrixNames.count(name) != 0;
};
