...
void ColorPalette::resizeGL(int w, int h)
{
    ...
    // H palette.
    {
        ...
        if ( m_H_indicator < 0 ) m_H_indicator = m_H_palette.y() + m_H_palette.height() / 2;
    }
    ...
    // SV palette
    {
        ...
        if ( m_S_indicator < 0 ) m_S_indicator = m_SV_palette.x() + m_SV_palette.width()*pixelRatio / 2;
        if ( m_V_indicator < 0 ) m_V_indicator = m_SV_palette.y() + m_SV_palette.height()*pixelRatio / 2;
    }
}
...
{
    return( static_cast<float>( m_S_indicator - m_SV_palette.x() ) / m_SV_palette.width() );
}
...
void ColorPalette::draw_selected_color_value( void )
{
    ...
    //kawamura
    char R[10]; sprintf( R, "R:%4.2f", (float)r/255 );
    char G[10]; sprintf( G, "G:%4.2f", (float)g/255 );
    char B[10]; sprintf( B, "B:%4.2f", (float)b/255 );
