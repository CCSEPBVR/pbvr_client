...
void TransferFuncEditor::importExtendTransferFunction(const ExtendedTransferFunctionMessage &doc){
    m_doc_initial = doc;
    m_doc = doc;
...
//    ui->color_function_synth->setText(QString::fromStdString(importdoc.m_color_transfer_function_synthesis) );
    ui->color_function_synth->setText(QString::fromStdString(doc.m_color_transfer_function_synthesis) );
//    ui->opacity_function_synth->setText(QString::fromStdString(importdoc.m_opacity_transfer_function_synthesis));
    ui->opacity_function_synth->setText(QString::fromStdString(doc.m_opacity_transfer_function_synthesis));
...
void TransferFuncEditor::displayTransferFunction()
{
    ...
    int n = ui->colormapFunction->currentIndex();
    ui->num_transfer->setValue(m_doc.m_transfer_function_number);
    this->populateColorFunctionLists(m_doc.m_transfer_function_number);
    this->populateOpacityFunctionLists(m_doc.m_transfer_function_number);
    ui->transfer_function_var_color->setText(m_doc.m_color_transfer_function[n].m_color_variable.c_str());
    ui->transfer_function_var_opacity->setText(m_doc.m_opacity_transfer_function[n].m_opacity_variable.c_str());



    if ( extCommand->m_result.m_color_bins.size() > n )
    {
        m_color_histogram->setTable( extCommand->m_result.m_color_bins[n] );
    }
    if ( extCommand->m_result.m_opacity_bins.size() > n )
    {
        m_opacity_histogram->setTable( extCommand->m_result.m_opacity_bins[n] );
    }
    m_color_histogram->update();
    m_opacity_histogram->update();
...
//    sprintf(tag_o, "%s_var_o", ui->opacitymapFunction->currentText().toStdString().c_str());
    ...
    if (trans_color != NULL && extCommand->m_result.m_color_bins.size() > n_select_color - 1) {
#if 0
...
#endif
        m_color_histogram->setTable(extCommand->m_result.m_color_bins[n_select_color - 1]);
        m_color_histogram->setRange(ui->transfer_function_min_color->value(), ui->transfer_function_max_color->value());
    }

    if (trans_opacity != NULL && extCommand->m_result.m_opacity_bins.size() > n_select_opacity - 1) {
#if 0
...
#endif
        m_opacity_histogram->setTable(extCommand->m_result.m_opacity_bins[n_select_opacity - 1]);
        m_opacity_histogram->setRange(ui->transfer_function_min_opacity->value(), ui->transfer_function_max_opacity->value());
    }

    std::cout << extCommand->m_result.m_var_range.min( tag_c ) << std::endl;
    std::cout << extCommand->m_result.m_var_range.max( tag_c ) << std::endl;
    std::cout << extCommand->m_result.m_var_range.min( tag_o ) << std::endl;
    std::cout << extCommand->m_result.m_var_range.max( tag_o ) << std::endl;
